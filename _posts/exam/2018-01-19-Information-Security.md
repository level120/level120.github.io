...
추가적으로 쓸데없는 비동기 연출을 나타내느라 코드가 좋지는 않습니다(특히 참고코드).
...
| 디자인 작업 | Bootstrap |
| 테스트 환경 | Chrome, Firefox, Edge, Whale, IE11(미지원) |
...
이렇게 들어온 10-bit의 키 값은 무작위로 자리를 바꾼다.
...
이렇게 얻은 키 값을 5-bit로 나누어 1-bit씩 왼쪽으로 옮겨야 한다(`LS`는 `Left Shift`의 줄임말).
...
이를 사용하지 않는 한국어, 일본어, 중국어 등을 사용하게 되면 문자 하나당 `2-byte`가 된다.

따라서 이를 제어하지 못한다면 영어와 숫자만 사용하도록 해야 한다.

(이 과제에는 이러한 내용이 적용되어 있지만 본 문서에서는 기술하지 않음, 참고 `js/func.js` -  `line no.222`)
...
다음은 `fk`과정이다. 순서로 따지면 위의 함수를 아래 코드에서 실행한다.
...
중간에 주석되어 있는 코드를 해제시키면 개발자 도구에서 로그값이 찍히는 것도 확인할 수 있다.
